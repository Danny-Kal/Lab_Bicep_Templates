...
try {
  ...
  # NEW: optional parametersUrl, coerced to string for safe serialization
  $parametersUrlString = [string]::Empty
  if ($input.parametersUrl -ne $null) {
    $parametersUrlString = $input.parametersUrl.ToString()
  }
  ...
  if (-not [string]::IsNullOrEmpty($templateUrlString)) {
    ...
    # NEW: only include parametersUrl if it was supplied
    if (-not [string]::IsNullOrEmpty($parametersUrlString)) {
      $deployInput.parametersUrl = $parametersUrlString
    }

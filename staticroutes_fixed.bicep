...
output publicWebIps array = [for (w, i) in webInstances: webPips[i].properties.ipAddress]
output webPipMap array = [
  for (w, i) in webInstances: {
    name: w.name
    ip:   webPips[i].properties.ipAddress
  }
]
